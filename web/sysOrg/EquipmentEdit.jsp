...
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <title>Prohealth - Edit Equipments</title>
                        ...
                        <div class="row-fluid">
                            <div class="span12" id="col0">
                                <div class="box" id="box-0">
                                    <h4 class="box-header round-top">Edit Equipment Requirements</h4>
                                    <div class="box-container-toggle">
                                        <div class="box-content">
                                            <form method="POST" action="Equipment" >
                                                <legend>Edit Equipment Requirements</legend>
                                                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered bootstrap-datatable" id="datatable1">
                                                    ...
                                                    %>
                                                    <tr>
                                                <input type="hidden" name="size" value="<%=equipmentList.size()%>"/>
                                                <input type="hidden" name="task_id-<%=x%>" value="<%=equipmentList.get(x).getTask_id()%>"/>
                                                <input type="hidden" name="title-<%=x%>" value="<%=equipmentList.get(x).getTitle()%>"/>
                                                <td><%=equipmentList.get(x).getTitle()%></td>
                                                <td><input type="number" name="currentcount-<%=x%>" value="<%=equipmentList.get(x).getCurrentcount()%>" class="input input-small"/></td>
                                                <td><input type="number" name="targetcount-<%=x%>" value="<%=equipmentList.get(x).getTargetcount()%>" class="input input-small"/></td>
                                                <td><input type="text" name="unit-<%=x%>" value="<%=equipmentList.get(x).getUnit()%>" class="input input-medium"/></td>
                                                <td>
