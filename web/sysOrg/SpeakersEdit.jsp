...
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <title>Prohealth - Edit Speakers</title>
                        ...
                        <div class="row-fluid">
                            <div class="span12" id="col0">
                                <div class="box" id="box-0">
                                    <h4 class="box-header round-top">Speaker Requirements</h4>
                                    <div class="box-container-toggle">
                                        <div class="box-content">
                                            <form method="POST" action="Speakers" >
                                                <legend>Edit Speaker Requirements</legend>
                                                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered bootstrap-datatable" id="datatable1">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th colspan="2">Current Number of Speakers</th>
                                                    ...
                                                    %>
                                                    <tr id="row-<%=speakerList.get(x).getTask_id()%>">
                                                <input type="hidden" name="size" value="<%=speakerList.size()%>"/>
                                                <input type="hidden" name="task_id-<%=x%>" value="<%=speakerList.get(x).getTask_id()%>"/>
                                                <input type="hidden" name="title-<%=x%>" value="<%=speakerList.get(x).getTitle()%>"/>
                                                <td><%=speakerList.get(x).getTitle()%></td>
                                                <td>
                                                    <input type="hidden" name="currentcount-<%=x%>" value="<%=speakerList.get(x).getCurrentcount()%>">
                                                    <%=speakerList.get(x).getCurrentcount()%>
                                                </td>
                                                <td>
                                                    <button name="action" value="increase-<%=x%>" type="submit" class="btn btn-success">
                                                        <i class="icon-plus icon-white"></i> Add Speaker
                                                    </button>
                                                    <br/>
                                                    <button style="margin-top: 5px" name="action" value="decrease-<%=x%>" type="submit" onclick="isSure()" class="btn btn-warning">
                                                        <i class="icon-minus icon-white"></i> Remove Speaker
                                                    </button>
                                                </td> 
                                                <td>
                                                    <input type="number" name="targetcount-<%=x%>" value="<%=speakerList.get(x).getTargetcount()%>" class="input input-small"/>
