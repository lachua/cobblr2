...
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <title>Prohealth - Remove Medical Professionals</title>
                        ...
                        <div class="row-fluid">
                            <div class="span12" id="col0">
                                <div class="box" id="box-0">
                                    <h4 class="box-header round-top">Delete Medical Practitioners</h4>
                                    <div class="box-container-toggle">
                                        <div class="box-content">
                                        <%String title = (String) request.getAttribute("title");%>
                                        <form method="POST" action="DoctorsIncDec" class="form-horizontal">
                                            <div class="control-group">
                                                <legend>Delete <%=title%>s</legend>
                                                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered bootstrap-datatable" id="datatable1">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        %>
                                                        <tr>
                                                            <td><%=medicalProfessionalList.get(x).getLastname()%>, <%=medicalProfessionalList.get(x).getFirstname()%> <%=medicalProfessionalList.get(x).getMi()%></td>
                                                            <td><%=medicalProfessionalList.get(x).getOrganizationname()%></td>
                                                            <td><%=medicalProfessionalList.get(x).getAddress()%></td>
                                                            <td><%=medicalProfessionalList.get(x).getEmail()%></td>
                                                            <td><%=medicalProfessionalList.get(x).getMobile()%></td>
                                                            <td>
                                                                <button name="action" value="delete-<%=medicalProfessionalList.get(x).getContact_id()%>" type="submit" class="btn btn-danger">
                                            ...
                                            <div class="form-actions">
                                                <a href="DocEditBack" type="button" class="btn">Back</a>
                                            </div>  
                                        </form>
                                    </div>
                                </div>
