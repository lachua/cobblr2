 ...
 * @author Renliw
 */
public class ProjectsApproved extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getMethod().equals("GET")) {
                ...
                List<UnavailableProjectEntity> currentProj = unavailableProjDAO.getPreactsProjects(UnavailableProjectDAO.APPROVED);
                unavailableProjDAO = new UnavailableProjectDAO();
                List<UnavailableProjectEntity> currentProjList = unavailableProjDAO.getPreactsProjectsCommunity(UnavailableProjectDAO.APPROVED);
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("/sysSLife/ProjectsApproved.jsp");
                dispatcher.forward(request, response);
            } else if (request.getMethod().equals("POST")) {
                if (request.getParameter("Unapprove") != null) {
                    int project_id = Integer.parseInt(request.getParameter("Unapprove"));
                    ProjectTaskDAO taskdao  = new ProjectTaskDAO();
                    boolean addDB = taskdao.setCompleted(project_id, "pre-acts", false);
                    ProjectCharterDAO projdao = new ProjectCharterDAO();
                    ProjectCharterEntity proj = projdao.getProjectCharter(project_id);
                    
                    request.setAttribute("projunapprove", proj.getTitle());
