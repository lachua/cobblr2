 ...
 * @author Renliw
 */
public class CloseProjects extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getMethod().equals("GET")) {
                ...
                List<UnavailableProjectEntity> closedProj = unavailableProjDAO.getAllProjectsTypeOrg(ProjectCharterDAO.IMPLMENTED, thisUser.getId());
                unavailableProjDAO = new UnavailableProjectDAO();
                List<UnavailableProjectEntity> closedProjList = unavailableProjDAO.getAllProjectsTypeOrgCommunity(ProjectCharterDAO.IMPLMENTED, thisUser.getId());

                request.setAttribute("closedProj", closedProj);
                request.setAttribute("closedProjList", closedProjList);
                RequestDispatcher dispatcher = request.getRequestDispatcher("/sysOrg/CloseProjects.jsp");
                dispatcher.forward(request, response);
            } else if (request.getMethod().equals("POST")) {
                if (request.getParameter("ViewClosedProjectCharter") != null) {
                    String project_id = request.getParameter("ViewClosedProjectCharter");
                    HttpSession session = request.getSession();
                ...
                } else if (request.getParameter("CloseSurvey") != null) {
                    String project_id = request.getParameter("CloseSurvey");
                    String[] id_type = project_id.split("-");
                    HttpSession session = request.getSession();
                    session.setAttribute("project_id", id_type[0]);
                    response.sendRedirect("SurveyOrg");
                }
            }
        } finally {
            out.close();
