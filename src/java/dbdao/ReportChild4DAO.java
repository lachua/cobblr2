...
import dbentities.ReportChild4Entity;
 ...
 * @author Renliw
 */
public class ReportChild4DAO extends QueryTemplate{

    // projects per status
    public List<ReportChild4Entity> getReport(int community_id, Date start, Date end){
        setQuery("SELECT \n" +
                "    oa.answertext as 'Vitamin',\n" +
                "    pop.total - count(ans.offeredanswer_id) as 'count'\n" +
                "FROM\n" +
                "	-- Possible choices\n" +
                "    (SELECT \n" +
        ...
        List<ReportChild4Entity> results = executeQuery();
    ...
    protected Object storeResults(ResultSet rs) {
        ReportChild4Entity entity = new ReportChild4Entity();

        try {
            entity.setVitamin(rs.getString("vitamin"));
